...
void relax(Graph G,int costPrev[],int u, int v, int weight){
    if(costPrev[u]+weight<G->vertex[v].cost){
        G->vertex[v].cost=costPrev[u]+weight;
        G->vertex[v].parent=u;
    }
}
...
void dist(Graph G,int k){
    ...
    //printf("the cost to CCCCCCCCCCCCCCCCCCCC: %d\n",G->digitsC);
...
int BellmanFord(Graph G, int s){
    ...
    for(pass=1; pass<G->numV;pass++){
        //store current ids and their cost to be used in each pass
        for(j=1;j<=G->numV;j++){
            costPrev[j]=G->vertex[j].cost;
        }

        for(j=1; j<=G->numV;j++){
            if(costPrev[j]!=Infinity){
                GEdgePtr p=G->vertex[j].firstEdge;

                while(p!=NULL){
                    relax(G,costPrev,j,p->child,p->weight);
    ...
    for(j=1;j<=G->numV;j++){
        GEdgePtr p=G->vertex[j].firstEdge;
        while(p!=NULL){
            if(costPrev[j]+p->weight<G->vertex[p->child].cost){
...
int main(){
    ...
    fp=fopen("inputBellmanFordAssign.txt","r");
